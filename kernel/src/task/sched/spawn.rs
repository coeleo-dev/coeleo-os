...
use super::pcb::{KernelCont, Pcb, Sched, State, TrapFrame, ERR, MAX_PROC, PS_REC, SCHED};
use super::switch::{
    CURRENT_SLOT, FXSAVES, FX_TEMPLATE, KCONTS, apply_hw, drop_pcb, enter_scheduler, fxsave_current,
    leave_scheduler, schedule_next, set_current_slot, yield_block, OUTCOME,
};
...
pub(super) fn run_with(image: Image, name: &str, exclusive: bool) -> Outcome {
    ...
    let Some(slot) = insert_pcb(image, name, 0) else {
...
pub(super) fn insert_pcb(image: Image, name: &str, parent: u32) -> Option<usize> {
    let mut s = SCHED.lock();
...
pub(super) fn spawn_with_parent(path: &str, parent: u32) -> Option<u32> {
    ...
    let slot = insert_pcb(image, path, parent)?;
