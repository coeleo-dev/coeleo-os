#!/usr/bin/env python3
"""Headless Fase 17.4: install GPT+ESP on a USB stick; OVMF boots the stick without ISO.

Live seed is the 17.2 GPT-FAT (type 0700) plus the all-hdd tree the engine copies.
The stick is qemu-xhci + usb-storage, not a second AHCI disk.
"""
...
def disk_kind_lines(serial: str, kind: str) -> list[str]:
    return [
        ln.strip()
        for ln in serial.splitlines()
        if ln.strip().startswith("disk:") and kind in ln and "MiB" in ln
    ]
...
def xhci_storage_args(stick: str, bootindex: bool) -> list[str]:
    args = [
        "-device",
        "qemu-xhci,id=xhci",
        "-drive",
        f"file={stick},if=none,format=raw,id=stick",
        "-device",
        "usb-storage,bus=xhci.0,drive=stick" + (",bootindex=1" if bootindex else ""),
    ]
    return args


def boot_live_and_stick(iso: str, live: str, stick: str) -> tuple[subprocess.Popen, socket.socket, str]:
    ...
    qemu = subprocess.Popen(
        [
            ...
            *xhci_storage_args(stick, bootindex=False),
            "-serial",
            f"file:{serial_log}",
            "-display",
            "none",
    ...
    try:
        ...
        serial = wait_count(serial_log, PROMPT, 1, 25)
...
def boot_stick_ovmf(ovmf: str, stick: str) -> tuple[subprocess.Popen, socket.socket, str]:
    work = os.path.dirname(os.path.abspath(stick))
    nonce = str(time.time_ns())
    ...
    qemu = subprocess.Popen(
        [
            ...
            *xhci_storage_args(stick, bootindex=True),
            "-net",
            "none",
            "-serial",
            f"file:{serial_log}",
...
def fail(msg: str, serial: str) -> int:
    print(f"test-phase17-usb: {msg}", file=sys.stderr)
...
def main() -> int:
    ...
    if not os.path.isfile(iso):
        print(f"test-phase17-usb: missing ISO {iso}", file=sys.stderr)
        return 1
    if not os.path.isfile(OVMF):
        print(f"test-phase17-usb: missing OVMF {OVMF}", file=sys.stderr)
        return 1

    with tempfile.TemporaryDirectory(prefix="coeleo-p17.4-") as tmp:
        live = os.path.join(tmp, "live.img")
        stick = os.path.join(tmp, "stick.img")
        make_live_image(live)
        make_blank_image(stick)

        qemu, sock, serial_log = boot_live_and_stick(iso, live, stick)
        serial = ""
        try:
            prompts = 1
            ...
            if "read failed" in serial:
                return fail("disk reported read failed", serial)
            if not disk_kind_lines(serial, "ahci"):
                return fail("disk did not list an ahci row", serial)
            if not disk_kind_lines(serial, "usb"):
                return fail("disk did not list a usb row", serial)
        ...
        stick_gpt = sgdisk_info(stick)
        if "EFI system" not in stick_gpt and "EF00" not in stick_gpt:
            return fail("stick is not ESP (EF00)", stick_gpt)

        env = mtools_env()
        listing = subprocess.run(
            ["mdir", "-i", f"{stick}@@1M", "::/EFI/BOOT"],
            check=True,
            capture_output=True,
            text=True,
            env=env,
        ...
        qemu, sock, serial_log = boot_stick_ovmf(OVMF, stick)
        serial = ""
        try:
            ...
            if "README.TXT" not in serial:
                return fail("ls did not list README.TXT on stick", serial)

            type_line(sock, "disk")
            serial = wait_count(serial_log, PROMPT, 3, 8)
            if not disk_kind_lines(serial, "usb"):
                return fail("disk did not list usb on stick", serial)
        finally:
            qemu_quit(qemu, sock)

        print("test-phase17-usb: ok")
        return 0
