#!/usr/bin/env python3
"""Fase 17.3: Install Coeleo GUI on the spare AHCI disk; OVMF boots the target.

Live seed is the 17.2 payload plus the install ELF. Headless live-root refuse
still holds; the wipe itself is driven through the wizard (run install).
"""
...
INSTALL_ELF = os.path.join(ROOT, "userspace", "apps", "install", "install")
...
def wizard_key(sock: socket.socket, key: str, pause: float = 0.35) -> None:
    send_keys(sock, [key])
    time.sleep(pause)
...
def make_live_image(path: str) -> None:
    """17.2 GPT-FAT 0700 payload plus the install wizard ELF."""
    require_file(SEED_README)
    require_file(SEED_HELLO)
    require_file(SH_ELF)
    require_file(INSTALL_ELF)
    require_file(KERNEL)
    ...
    subprocess.run(["mcopy", "-i", f"{path}@@1M", INSTALL_ELF, "::install"], check=True, env=env)
...
def fail(msg: str, serial: str) -> int:
    print(f"test-phase17: {msg}", file=sys.stderr)
...
def main() -> int:
    ...
    if not os.path.isfile(iso):
        print(f"test-phase17: missing ISO {iso}", file=sys.stderr)
        return 1
    if not os.path.isfile(OVMF):
        print(f"test-phase17: missing OVMF {OVMF}", file=sys.stderr)
        return 1
    if not os.path.isfile(INSTALL_ELF):
        print(f"test-phase17: missing install ELF {INSTALL_ELF}", file=sys.stderr)
        return 1

    with tempfile.TemporaryDirectory(prefix="coeleo-p17-") as tmp:
        ...
        try:
            ...
            type_line(sock, "run install")
            serial = wait_count(serial_log, "win: create", 1, 12)
            if "win: create" not in serial:
                return fail("run install did not create a window", serial)
            time.sleep(0.4)
            ...
            if "install: confirm" in serial:
                return fail("Erase Cancel still armed or wrote", serial)
            if "install: ok" in serial:
                return fail("Erase Cancel wrote the disk", serial)

            wizard_key(sock, "ret")
            wizard_key(sock, "right")
            wizard_key(sock, "ret", pause=0.2)
            serial = wait_count(serial_log, "install: confirm", 1, 8)
            if "install: confirm" not in serial:
                return fail("Erase disk did not print install: confirm", serial)
            serial = wait_count(serial_log, "install: ok", 1, 180)
            if "install: failed" in serial:
                return fail("GUI install failed", serial)
            if "install: ok" not in serial:
                return fail("GUI install did not finish", serial)
        finally:
            qemu_quit(qemu, sock)
