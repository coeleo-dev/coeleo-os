#!/usr/bin/env python3
"""Headless Fase 35: boot with -smp 2, verify APs come up and the shell works."""
...
def make_fat_image(path: str, sh_elf: str, clock_elf: str, hello_elf: str) -> None:
    if not os.path.isfile(SEED_README):
        raise FileNotFoundError("disk-seed/README.TXT")
    for p in (sh_elf, clock_elf, hello_elf):
...
def fail(msg: str, serial: str) -> int:
    print(f"test-phase35: {msg}", file=sys.stderr)
...
def main() -> int:
    if len(sys.argv) != 5:
        print(f"usage: {sys.argv[0]} coeleo.iso sh_elf clock_elf hello_elf", file=sys.stderr)
        return 2
    iso, sh_elf, clock_elf, hello_elf = sys.argv[1:5]
    if not os.path.isfile(iso):
        print(f"test-phase35: missing ISO {iso}", file=sys.stderr)
        return 1

    with tempfile.TemporaryDirectory(prefix="coeleo-p35-") as tmp:
        serial_log = os.path.join(tmp, "serial.log")
        qmp_path = os.path.join(tmp, "qmp.sock")
        disk = os.path.join(tmp, "disk.img")
        make_fat_image(disk, sh_elf, clock_elf, hello_elf)
        serial = ""
        ...
        try:
            ...
            try:
                ...
                # APs must have reported in.
                serial = wait_contains(serial_log, "smp:", 5)
                if "smp:" not in serial:
                    return fail("no smp: marker in serial", serial)
