#!/usr/bin/env python3
"""Headless Plasma P2: run winprobe; serial has win: create and blit: 64x64."""
...
def make_fat_image(path: str, sh_elf: str, winprobe_elf: str) -> None:
    ...
    if not os.path.isfile(winprobe_elf):
        raise FileNotFoundError(winprobe_elf)
    env = os.environ.copy()
    ...
    subprocess.run(["mcopy", "-i", path, winprobe_elf, "::winprobe"], check=True, env=env)


def fail(msg: str, serial: str) -> int:
    print(f"test-plasma-p2: {msg}", file=sys.stderr)
...
def main() -> int:
    if len(sys.argv) != 4:
        print(f"usage: {sys.argv[0]} coeleo.iso sh_elf winprobe_elf", file=sys.stderr)
        return 2
    iso, sh_elf, winprobe_elf = sys.argv[1], sys.argv[2], sys.argv[3]
    if not os.path.isfile(iso):
        print(f"test-plasma-p2: missing ISO {iso}", file=sys.stderr)
        return 1
    ...
    with tempfile.TemporaryDirectory(prefix="coeleo-p2-") as tmp:
        ...
        make_fat_image(disk, sh_elf, winprobe_elf)
        serial = ""
        ...
        try:
            ...
            try:
                ...
                serial = wait_file_contains(serial_log, "blit: 64x64", 5)
                if "blit: 64x64" not in serial:
                    return fail("missing blit: 64x64", serial)
                serial = wait_prompt_count(serial_log, 2, 8)
                if serial.count(PROMPT) < 2:
                    return fail("prompt did not return after winprobe", serial)
            finally:
                sock.close()
        finally:
            qemu.terminate()
