#!/usr/bin/env python3
"""Headless Fase 9: boot into ELF sh; ls/cat/cd/exit; prompt respawns."""
...
def make_fat_image(path: str, sh_elf: str, ls_elf: str, cat_elf: str) -> None:
    ...
    for p in (sh_elf, ls_elf, cat_elf):
    ...
    subprocess.run(["mcopy", "-i", path, ls_elf, "::ls"], check=True, env=env)
    subprocess.run(["mcopy", "-i", path, cat_elf, "::cat"], check=True, env=env)


def fail(msg: str, serial: str) -> int:
    print(f"test-phase9: {msg}", file=sys.stderr)
...
def main() -> int:
    if len(sys.argv) != 5:
        print(f"usage: {sys.argv[0]} coeleo.iso sh_elf ls_elf cat_elf", file=sys.stderr)
        return 2
    iso, sh_elf, ls_elf, cat_elf = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
    if not os.path.isfile(iso):
        print(f"test-phase9: missing ISO {iso}", file=sys.stderr)
        return 1

    with tempfile.TemporaryDirectory(prefix="coeleo-p9-") as tmp:
        serial_log = os.path.join(tmp, "serial.log")
        qmp_path = os.path.join(tmp, "qmp.sock")
        disk = os.path.join(tmp, "disk.img")
        make_fat_image(disk, sh_elf, ls_elf, cat_elf)
        serial = ""
        ...
        try:
            ...
            try:
                ...
                if serial.count(PROMPT) < prompts:
                    return fail("timed out after exit (respawn)", serial)

                type_line(sock, "ls")
                prompts += 1
                serial = wait_prompt_count(serial_log, prompts, 8)
                if "README.TXT" not in serial:
                    return fail("ls after exit did not list README.TXT", serial)
            finally:
                sock.close()
        finally:
            qemu.terminate()
