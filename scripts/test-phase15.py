#!/usr/bin/env python3
"""Headless Fase 15: AHCI SATA (GPT FAT32), no virtio; disk/ls/write/sync/reboot."""
...
def fail(msg: str, serial: str) -> int:
    print(f"test-phase15: {msg}", file=sys.stderr)
...
def main() -> int:
    ...
    if not os.path.isfile(iso):
        print(f"test-phase15: missing ISO {iso}", file=sys.stderr)
        return 1

    with tempfile.TemporaryDirectory(prefix="coeleo-p15-") as tmp:
        disk = os.path.join(tmp, "disk.img")
        make_gpt_fat_image(disk)
        ...
        try:
            ...
            if "ahci" not in serial or "MiB" not in serial:
                return fail("disk did not report ahci capacity", serial)
        ...
        if "write" not in serial or "ping" not in serial:
            return fail("ELF help did not list write and ping", serial)
        print("test-phase15: ok")
        return 0
