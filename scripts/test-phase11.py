#!/usr/bin/env python3
"""Headless Fase 11: QMP ping to QEMU SLIRP gateway 10.0.2.2 over virtio-net."""
...
def fail(msg: str, serial: str) -> int:
    print(f"test-phase11: {msg}", file=sys.stderr)
...
def main() -> int:
    ...
    if not os.path.isfile(iso):
        print(f"test-phase11: missing ISO {iso}", file=sys.stderr)
        return 1

    with tempfile.TemporaryDirectory(prefix="coeleo-p11-") as tmp:
        ...
        try:
            ...
            try:
                ...
                serial = wait_file_contains(serial_log, REPLY, 20)
