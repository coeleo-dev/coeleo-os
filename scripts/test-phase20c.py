#!/usr/bin/env python3
"""Headless sh daily: mkdir, echo redirect, pwd."""
...
def make_fat_image(path: str, sh_elf: str, echo_elf: str, cat_elf: str) -> None:
    if not os.path.isfile(SEED_README):
        raise FileNotFoundError("disk-seed/README.TXT")
    for p in (sh_elf, echo_elf, cat_elf):
    ...
    subprocess.run(["mcopy", "-i", path, echo_elf, "::echo"], check=True, env=env)
    subprocess.run(["mcopy", "-i", path, cat_elf, "::cat"], check=True, env=env)


def fail(msg: str, serial: str) -> int:
    print(f"test-phase20c: {msg}", file=sys.stderr)
...
def main() -> int:
    if len(sys.argv) != 5:
        print(
            f"usage: {sys.argv[0]} coeleo.iso sh_elf echo_elf cat_elf",
            file=sys.stderr,
        )
        return 2
    iso, sh_elf, echo_elf, cat_elf = sys.argv[1:5]
    if not os.path.isfile(iso):
        print(f"test-phase20c: missing ISO {iso}", file=sys.stderr)
        return 1

    with tempfile.TemporaryDirectory(prefix="coeleo-p20c-") as tmp:
        serial_log = os.path.join(tmp, "serial.log")
        qmp_path = os.path.join(tmp, "qmp.sock")
        disk = os.path.join(tmp, "disk.img")
        make_fat_image(disk, sh_elf, echo_elf, cat_elf)
        serial = ""
        ...
        try:
            ...
            try:
                ...
                type_line(sock, "mkdir d")
                prompts += 1
                serial = wait_prompt_count(serial_log, prompts, 8)
                if serial.count(PROMPT) < prompts:
                    return fail("timed out after mkdir", serial)
                if "mkdir: failed" in serial:
                    return fail("mkdir d failed", serial)

                type_line(sock, "echo hi > d/a")
                prompts += 1
                serial = wait_prompt_count(serial_log, prompts, 8)
                if serial.count(PROMPT) < prompts:
                    return fail("timed out after echo redirect", serial)

                type_line(sock, "cat d/a")
                prompts += 1
                serial = wait_prompt_count(serial_log, prompts, 8)
                if serial.count(PROMPT) < prompts:
                    return fail("timed out after cat d/a", serial)
                if "hi" not in serial:
                    return fail("cat d/a missing hi", serial)

                type_line(sock, "pwd")
                prompts += 1
                serial = wait_prompt_count(serial_log, prompts, 5)
                if serial.count(PROMPT) < prompts:
                    return fail("timed out after pwd", serial)
                if "pwd: " in serial:
                    return fail("pwd printed an error", serial)
            finally:
                sock.close()
        finally:
            qemu.terminate()
